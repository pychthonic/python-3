...
"""
This program uses Requests to check the accuracy of ordinalize.py, also found in this 
repository. It uses requests to go to http://www.isthe.com/cgi-bin/chongo/number.cgi 
and submit the number input by the user to the form there. It then downloads the HTTP 
response, parses it for and removes the number, ordinalizes the number (five -> fifth, 
etc) and then uses ordinalize.py as a module to get what should be the same number. 
It lastly removes punctuation, checks whether the two strings are equal, and lets the 
user know either way.
